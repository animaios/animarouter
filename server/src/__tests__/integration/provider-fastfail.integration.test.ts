...
describe("Provider-Outage Fast-Fail — Integration", () => {
  ...
  beforeEach(async () => {
    ...
    db.prepare(
      "INSERT INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, enabled) VALUES ('alpha', 'model-a1', 'Alpha A1', 1, 1, 1)",
    ).run();
    db.prepare(
      "INSERT INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, enabled) VALUES ('alpha', 'model-a2', 'Alpha A2', 2, 2, 1)",
    ).run();
    db.prepare(
      "INSERT INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, enabled) VALUES ('beta', 'model-b1', 'Beta B1', 3, 3, 1)",
    ).run();

    alphaModel1Id = (
      db
        .prepare(
          "SELECT id FROM models WHERE platform = 'alpha' AND model_id = 'model-a1'",
        )
        .get() as any
    ).id;
    alphaModel2Id = (
      db
        .prepare(
          "SELECT id FROM models WHERE platform = 'alpha' AND model_id = 'model-a2'",
        )
        .get() as any
    ).id;
    betaModel1Id = (
      db
        .prepare(
          "SELECT id FROM models WHERE platform = 'beta' AND model_id = 'model-b1'",
        )
        .get() as any
    ).id;
    ...
    db.prepare(
      "INSERT INTO api_keys (platform, label, encrypted_key, iv, auth_tag, status, enabled) VALUES ('alpha', 'Alpha Key', ?, ?, ?, 'healthy', 1)",
    ).run(encrypted, iv, authTag);
    db.prepare(
      "INSERT INTO api_keys (platform, label, encrypted_key, iv, auth_tag, status, enabled) VALUES ('beta', 'Beta Key', ?, ?, ?, 'healthy', 1)",
    ).run(encrypted, iv, authTag);
  });
  ...
  it("2 distinct models with 5xx on same provider triggers fast-fail — routes to fallback provider", async () => {
    ...
    chatCompletion
      .mockRejectedValueOnce(
        new Error("alpha API error 503: Service Unavailable"),
      )
      .mockRejectedValueOnce(
        new Error("alpha API error 503: Service Unavailable"),
      )
    ...
    const fastFailEvents = publishedEvents.filter(
      (e) => e.type === "routing.provider_fastfail",
    );
  ...
  it("single model 5xx does NOT trigger fast-fail (below threshold)", async () => {
    ...
    chatCompletion
      ...
      .mockResolvedValueOnce({
        choices: [
          { message: { role: "assistant", content: "alpha a2 answer" } },
        ],
        usage: { prompt_tokens: 10, completion_tokens: 3, total_tokens: 13 },
      });
  ...
  it("key_exhausted events are emitted before fast-fail event", async () => {
    ...
    const exhaustedEvents = publishedEvents.filter(
      (e) => e.type === "routing.key_exhausted",
    );
    const fastFailEvents = publishedEvents.filter(
      (e) => e.type === "routing.provider_fastfail",
    );
