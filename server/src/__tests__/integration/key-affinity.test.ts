...
import { routeRequest, type RouteOptions } from '../../services/router.js';
...
describe('Key Affinity Per Thread', () => {
    ...
    `).run();
        ...
        const modelId = db.prepare('SELECT id FROM models WHERE model_id = ?').get('test-model') as { id: number };
    ...
    describe('Happy Path: Same Session → Same Key', () => {
        it('should select the same key for requests with the same session key', () => {
            ...
            const route1 = routeRequest(1000, undefined, undefined, false, false, undefined, options);
            const keyId1 = route1.keyId;
            route1.release();

            const route2 = routeRequest(1000, undefined, undefined, false, false, undefined, options);
            const keyId2 = route2.keyId;
            route2.release();

            const route3 = routeRequest(1000, undefined, undefined, false, false, undefined, options);
        ...
        it('should compute deterministic key selection based on hash', () => {
            ...
            for (const sessionKey of sessions) {
                const options: RouteOptions = { stickySessionKey: sessionKey };
                const route = routeRequest(1000, undefined, undefined, false, false, undefined, options);
    ...
    describe('Happy Path: Different Sessions → Different Keys', () => {
        it('should select keys deterministically for different sessions', () => {
            ...
            const route1 = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: sessionKey1 });
            const keyId1 = route1.keyId;
            route1.release();

            const route2 = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: sessionKey2 });
            const keyId2 = route2.keyId;
            route2.release();

            // Verify the selection is deterministic
            const route1Again = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: sessionKey1 });
            const route2Again = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: sessionKey2 });
        ...
        it('should distribute sessions across available keys', () => {
            ...
            for (let i = 0; i < numSessions; i++) {
                ...
                const route = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: sessionKey });
    ...
    describe('Happy Path: Disabled Mode → Round-Robin', () => {
        ...
        it('should ignore stickySessionKey when affinity is disabled', () => {
            ...
            const route1 = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: 'session-1' });
            const keyId1 = route1.keyId;
            route1.release();

            const route2 = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: 'session-2' });
            const keyId2 = route2.keyId;
            route2.release();

            const route3 = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: 'session-3' });
            const keyId3 = route3.keyId;
            route3.release();
    ...
    describe('Edge Case: Exhausted Key Fallback', () => {
        it('should fall back to another key when the affinity key is exhausted', () => {
            const sessionKey = crypto.createHash('sha1').update('test-session').digest('hex');
            const options: RouteOptions = { stickySessionKey: sessionKey };
            ...
            const exhausted = new Set<string>([`test-provider:test-model:${originalKeyId}`]);

            // Next request should fall back to a different key
            const route2 = routeRequest(1000, exhausted, undefined, false, false, undefined, options);
        ...
        it('should try all keys before giving up', () => {
            ...
            // Get all key IDs
            const allKeyIds = (db.prepare('SELECT id FROM api_keys').all() as Array<{ id: number }>).map(k => k.id);
            ...
            const route = routeRequest(1000, exhausted, undefined, false, false, undefined, options);
        ...
        it('should throw when all keys are exhausted', () => {
            ...
            expect(() => {
                routeRequest(1000, exhausted, undefined, false, false, undefined, options);
            }).toThrow();
        });
    });
    ...
    describe('Edge Case: Empty Session Key', () => {
        it('should fall back to round-robin when session key is empty', () => {
            ...
            for (let i = 0; i < 6; i++) {
                const route = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: '' });
                keyIds.push(route.keyId);
                route.release();
            }
        ...
        it('should fall back to round-robin when stickySessionKey is undefined', () => {
            ...
            for (let i = 0; i < 6; i++) {
                const route = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: undefined });
                keyIds.push(route.keyId);
                route.release();
            }
    ...
    describe('Edge Case: Pin Mode with Affinity', () => {
        it('should respect pin mode and still use affinity for key selection', () => {
            const modelId = (db.prepare('SELECT id FROM models WHERE model_id = ?').get('test-model') as { id: number }).id;
            const sessionKey = 'pinned-session';

            const route1 = routeRequest(1000, undefined, modelId, false, false, undefined, { pinMode: true, stickySessionKey: sessionKey });
            const keyId1 = route1.keyId;
            route1.release();

            const route2 = routeRequest(1000, undefined, modelId, false, false, undefined, { pinMode: true, stickySessionKey: sessionKey });
            const keyId2 = route2.keyId;
            route2.release();
    ...
    describe('Edge Case: Hash Distribution', () => {
        it('should distribute hashes evenly across keys', () => {
            ...
            for (let i = 0; i < numSessions; i++) {
                const sessionKey = crypto.createHash('sha1').update(`unique-session-${i}`).digest('hex');
                const route = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: sessionKey });
    ...
    describe('Edge Case: Backward Compatibility with Provider Sticky Sessions', () => {
        it('should use provider-level sticky_sessions_enabled when global affinity is off', () => {
            ...
            db.prepare('UPDATE custom_providers SET sticky_sessions_enabled = 1 WHERE slug = ?').run('test-provider');
        ...
        it('should prioritize global affinity over provider-level setting', () => {
            ...
            db.prepare('UPDATE custom_providers SET sticky_sessions_enabled = 0 WHERE slug = ?').run('test-provider');
    ...
    describe('Edge Case: Session Key Consistency', () => {
      ...
      `).run();
            ...
            const model2Id = (db.prepare('SELECT id FROM models WHERE model_id = ?').get('test-model-2') as { id: number }).id;
      ...
      `).run(model2Id);
            ...
            const route1 = routeRequest(1000, undefined, undefined, false, false, undefined, { stickySessionKey: sessionKey });
            const keyId1 = route1.keyId;
            route1.release();

            // Skip the first model, forcing fallback to second model
            const modelId1 = (db.prepare('SELECT id FROM models WHERE model_id = ?').get('test-model') as { id: number }).id;
            const skipModels = new Set([modelId1]);

            const route2 = routeRequest(1000, undefined, undefined, false, false, skipModels, { stickySessionKey: sessionKey });
            const keyId2 = route2.keyId;
            route2.release();
