...
describe("resolveGroupKey", () => {
  it("normalizes model ID and returns it as group_key when no alias matches", () => {
    ...
    expect(resolveGroupKey("meta-llama/llama-3.3-70b-instruct", cache)).toBe(
      "llama-3-3-70b",
    );
  });
  ...
  it("returns the alias target group_key when the normalized ID matches an alias", () => {
    ...
    expect(resolveGroupKey("deepseek-v4-flash-free", cache)).toBe(
      "deepseek-v4-flash",
    );
  });
  ...
  it("chains aliases: normalized ID maps to alias target which may itself be a group_key", () => {
    ...
    const cache = new Map<string, string>([["gpt-4o-2024-11-20", "gpt-4o"]]);
...
describe("alias cache", () => {
  ...
  it("seeds default aliases for OpenCode Zen provider variants", () => {
    ...
    expect(cache.get("nemotron-3-ultra-free")).toBe(
      "nemotron-3-ultra-550b-a55b",
    );
    expect(cache.get("nemotron-3-ultra-550b-a55b:free")).toBe(
      "nemotron-3-ultra-550b-a55b",
    );
  });
  ...
  it("applies default display names for canonical grouped models", () => {
    ...
    const group = db
      .prepare("SELECT display_name FROM model_groups WHERE group_key = ?")
...
describe("reconcileGroups", () => {
  ...
  it("creates groups for models that do not have one", () => {
    ...
    const row = db
      ...
      .get("gpt-4o") as { group_id: number | null };
    expect(row.group_id).not.toBeNull();

    // A group should exist
    const group = db
      .prepare("SELECT * FROM model_groups WHERE group_key = ?")
      .get("gpt-4o") as any;
  ...
  it("reassigns models with stale group_ids", () => {
    ...
    const row = db
      .prepare("SELECT group_id FROM models WHERE model_id = ?")
      .get("gpt-4o") as { group_id: number };
    const correctGroup = db
      .prepare("SELECT id FROM model_groups WHERE group_key = ?")
      .get("gpt-4o") as { id: number };
...
describe("ensureModelInGroup", () => {
  ...
  it("returns the group_id", () => {
    ...
    const model = db
      .prepare("SELECT id FROM models WHERE model_id = ?")
      .get("gemini-2.5-pro") as { id: number };
    const groupId = ensureModelInGroup(db, model.id);

    const row = db
      .prepare("SELECT group_id FROM models WHERE id = ?")
      .get(model.id) as { group_id: number };
...
describe("propagateGroupProperties", () => {
  ...
  it("writes group properties to all member model rows", () => {
    ...
    const models = db
      .prepare("SELECT * FROM models WHERE group_id = ?")
      .all(group.id) as any[];
