...
describe("intelligence tier audit (migrateModelsV17)", () => {
  ...
  function tier(platform: string, modelId: string): string | undefined {
    const row = getDb()
      .prepare(
        "SELECT size_label FROM models WHERE platform = ? AND model_id = ?",
      )
  ...
  function tiersForFamily(like: string): string[] {
    const rows = getDb()
      .prepare(
        "SELECT DISTINCT size_label FROM models WHERE LOWER(model_id) LIKE ?",
      )
  ...
  it("demotes over-tiered models down from Frontier", () => {
    ...
    expect(tier("nvidia", "qwen/qwen3-coder-480b-a35b-instruct")).toBe(
      "Medium",
    );
  ...
  it("leaves models with no published AA Index at their seeded tier", () => {
    ...
    expect(
      tier("openrouter", "nousresearch/hermes-3-llama-3.1-405b:free"),
    ).toBe("Large");
  });

  it("applies curated benchmark overrides after the static V17 tier audit", () => {
    expect(tier("openrouter", "poolside/laguna-m.1:free")).toBe("Frontier");
  });
