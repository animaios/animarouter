...
describe("Migration idempotency", () => {
  it("initDb on a fresh in-memory DB then re-run produces identical row counts", () => {
    ...
    const before = {
      models: (
        db1.prepare("SELECT COUNT(*) AS c FROM models").get() as { c: number }
      ).c,
      fallback: (
        db1.prepare("SELECT COUNT(*) AS c FROM fallback_config").get() as {
          c: number;
        }
      ).c,
      orphanFallbacks: (
    ...
    const after = {
      models: (
        db2.prepare("SELECT COUNT(*) AS c FROM models").get() as { c: number }
      ).c,
      fallback: (
        db2.prepare("SELECT COUNT(*) AS c FROM fallback_config").get() as {
          c: number;
        }
      ).c,
      orphanFallbacks: (
  ...
  it("V38 preserves fallback rows while normalizing one grouped fallback per group", () => {
    ...
    db.prepare(
      "DROP INDEX IF EXISTS idx_fallback_config_group_id_unique",
    ).run();
    ...
    const tieGroupId = insertGroup.run(`v38-tie-${suffix}`, "V38 Tie")
      .lastInsertRowid as number;
    ...
    expect(
      priorityRows.filter((row) => row.group_id === priorityGroupId),
    ).toEqual([
      {
        model_db_id: lowPriorityModelId,
        priority: 10,
  ...
  it("adds group_id columns before V38 runs on legacy schemas", () => {
    ...
    const db2 = initDb(tmpPath);
    const upgradedModelColumns = (
      db2.prepare("PRAGMA table_info(models)").all() as Array<{ name: string }>
    ).map((col) => col.name);
    const upgradedFallbackColumns = (
      db2.prepare("PRAGMA table_info(fallback_config)").all() as Array<{
        name: string;
      }>
    ).map((col) => col.name);
    ...
    expect(
      (
        db2.prepare("SELECT COUNT(*) AS c FROM model_groups").get() as {
          c: number;
        }
      ).c,
    ).toBeGreaterThan(0);
    expect(
      (
        db2
          .prepare(
            "SELECT COUNT(*) AS c FROM models WHERE enabled = 1 AND group_id IS NULL",
          )
          .get() as { c: number }
      ).c,
    ).toBe(0);
  ...
  it("manual benchmark overrides pin the curated top-pool intelligence order", () => {
    ...
    const expected: Array<{
    ...
    }> = [
      {
        label: "glm-5-1",
        score: 100,
        modelKeys: ["glm-5-1", "glm-5-1-fp8"],
        groupKeys: ["glm-5-1", "glm-5-1-fp8"],
      },
      {
        label: "kimi-k2-6",
        score: 93,
        modelKeys: ["@cf/moonshotai/kimi-k2-6", "kimi-k2-6", "kimi-k2-6:free"],
        groupKeys: ["@cf/moonshotai/kimi-k2-6", "kimi-k2-6", "kimi-k2-6:free"],
      },
      {
        label: "nemotron-3-ultra",
        score: 89,
        modelKeys: ["nemotron-3-ultra-550b-a55b:free", "nemotron-3-ultra-free"],
        groupKeys: ["nemotron-3-ultra-550b-a55b", "nemotron-3-ultra-free"],
      },
      {
        label: "minimax-m2-7",
        score: 85,
        modelKeys: ["minimax-m2-7"],
        groupKeys: ["minimax-m2-7"],
      },
      {
        label: "deepseek-v4-flash",
        score: 62,
        modelKeys: ["deepseek-v4-flash", "deepseek-v4-flash-free"],
        groupKeys: [
          "deepseek-v4-flash",
      ...
      },
      {
        label: "minimax-m3",
        score: 98,
        modelKeys: ["minimax-m3-free"],
        groupKeys: ["minimax-m3"],
      },
      {
        label: "mimo-v2.5-free",
        score: 74,
        modelKeys: ["mimo-v2-5-free"],
        groupKeys: ["mimo-v2-5-free"],
      },
      {
        label: "laguna-m-1",
        score: 74,
        modelKeys: ["laguna-m-1:free"],
        groupKeys: ["laguna-m-1:free"],
      },
      {
        label: "step-3-7-flash",
        score: 65,
        modelKeys: ["step-3-7-flash:free"],
        groupKeys: ["step-3-7-flash:free"],
      },
    ];
    ...
    for (const { label, score, modelKeys, groupKeys } of expected) {
      ...
      expect(
        rows.every((r) => r.benchmark_score === score),
        label,
      ).toBe(true);
      expect(
        rows.every(
          (r) => r.intelligence_rank === scoreToIntelligenceRank(score),
        ),
        label,
      ).toBe(true);
      expect(
        rows.every((r) => r.size_label === "Frontier"),
        label,
      ).toBe(true);
      ...
      expect(
        groups.every((g) => g.benchmark_score === score),
        label,
      ).toBe(true);
      expect(
        groups.every(
          (g) => g.intelligence_rank === scoreToIntelligenceRank(score),
        ),
        label,
      ).toBe(true);
      expect(
        groups.every((g) => g.size_label === "Frontier"),
        label,
      ).toBe(true);
    }
  });
  ...
  it("all enabled catalog platforms have a registered provider", async () => {
    ...
    const platforms = (
      db
        .prepare(`SELECT DISTINCT platform FROM models WHERE enabled = 1`)
        .all() as { platform: any }[]
    ).map((r) => r.platform);
