...
describe("Tools-aware routing", () => {
  ...
  it("flags tool-capable families and leaves the known-bad ones unflagged", () => {
    ...
    const flag = (modelId: string) =>
      (
        db
          .prepare("SELECT supports_tools FROM models WHERE model_id = ?")
          .get(modelId) as { supports_tools: number } | undefined
      )?.supports_tools;
    ...
    const hermes = db
      .prepare(
        "SELECT supports_tools FROM models WHERE model_id LIKE '%hermes-3%'",
      )
      .all() as { supports_tools: number }[];
    for (const h of hermes) expect(h.supports_tools).toBe(0);

    // gemma must NOT ride the gemini rule.
    const gemma = db
      .prepare(
        "SELECT supports_tools FROM models WHERE LOWER(model_id) LIKE '%gemma%'",
      )
      .all() as { supports_tools: number }[];
    ...
    const on = (
      db
        .prepare("SELECT COUNT(*) c FROM models WHERE supports_tools = 1")
        .get() as { c: number }
    ).c;
    const off = (
      db
        .prepare("SELECT COUNT(*) c FROM models WHERE supports_tools = 0")
        .get() as { c: number }
    ).c;
  ...
  it("routeRequest skips non-tool models when requireTools is set", () => {
    ...
    const gemma = db
      .prepare(
        "SELECT id FROM models WHERE platform = 'google' AND LOWER(model_id) LIKE '%gemma%' AND enabled = 1",
      )
      .get() as { id: number } | undefined;
    expect(gemma).toBeDefined();
    db.prepare(
      "UPDATE fallback_config SET priority = 0, enabled = 1 WHERE model_db_id = ?",
    ).run(gemma!.id);
    ...
    const flag = db
      .prepare("SELECT supports_tools FROM models WHERE id = ?")
      .get(tooled.modelDbId) as { supports_tools: number };
  ...
  it("rejects a tool request with a clear 422 when no tool-capable model is enabled", async () => {
    ...
    getDb()
      .prepare(
        "UPDATE models SET supports_tools = 1 WHERE model_id IN ('gemini-3.5-flash', 'gemini-3.5-pro', 'gpt-oss-120b', 'glm-4.7')",
      )
      .run();
  });
