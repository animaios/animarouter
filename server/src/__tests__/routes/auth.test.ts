...
describe("Dashboard auth (#35)", () => {
  ...
  it("leaves /api/ping and the /v1 proxy reachable without a dashboard session", async () => {
    ...
    const proxy = await call(app, "POST", "/v1/chat/completions", {
  ...
  it("rejects weak setup credentials", async () => {
    expect(
      (
        await call(app, "POST", "/api/auth/setup", {
          email: "bad",
          password: "x",
        })
      ).status,
  ...
  it("creates the first account on setup and returns a working token", async () => {
    const { status, body } = await call(app, "POST", "/api/auth/setup", {
    ...
    token = body.token;
    expect((await call(app, "GET", "/api/keys", undefined, token)).status).toBe(
      200,
    );
  });

  it("refuses a second setup once an account exists", async () => {
    const { status } = await call(app, "POST", "/api/auth/setup", {
  ...
  it("logs in with correct credentials and rejects wrong ones", async () => {
    const ok = await call(app, "POST", "/api/auth/login", {
      email: "admin@example.com",
      password: "supersecret",
    });
    ...
    const bad = await call(app, "POST", "/api/auth/login", {
      email: "admin@example.com",
  ...
  it("invalidates the token on logout", async () => {
    const login = await call(app, "POST", "/api/auth/login", {
      email: "admin@example.com",
      password: "supersecret",
    });
    const t = login.body.token;
    expect((await call(app, "GET", "/api/keys", undefined, t)).status).toBe(
      200,
    );
  ...
  it("locks out after repeated failed attempts (separate email, no real account)", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      expect((await call(app, "POST", "/api/auth/login", creds)).status).toBe(
        401,
      );
    }
    expect((await call(app, "POST", "/api/auth/login", creds)).status).toBe(
