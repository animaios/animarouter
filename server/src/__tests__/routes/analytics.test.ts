...
describe("Analytics API", () => {
  ...
  describe("pinned vs auto tracking", () => {
    ...
    it("by-model counts only requests the model served because it was pinned", async () => {
      ...
      const { status, body } = await request(
        app,
        "/api/analytics/by-model?range=24h",
      );
  ...
  describe("active provider filtering", () => {
    ...
    it("includes requests once a key is enabled", async () => {
      ...
      getDb()
        .prepare("UPDATE api_keys SET enabled = 1 WHERE platform = ?")
        .run("late");
    ...
    it("filters by-platform endpoint", async () => {
      ...
      const { body } = await request(
        app,
        "/api/analytics/by-platform?range=24h",
      );
    ...
    it("filters error-distribution endpoint", async () => {
      ...
      const { body } = await request(
        app,
        "/api/analytics/error-distribution?range=24h",
      );
    ...
    it("filters model-timeline endpoint", async () => {
      ...
      const { body } = await request(
        app,
        "/api/analytics/model-timeline?range=24h",
      );

      expect(body.series).toHaveLength(1);
      expect(body.series[0].platform).toBe("mtactive");
      const nonZeroBuckets = body.points.filter(
        (pt: any) => pt.totalRequests > 0,
      );
      expect(nonZeroBuckets.length).toBeGreaterThan(0);
      expect(nonZeroBuckets.every((pt: any) => pt.totalRequests === 1)).toBe(
        true,
      );
    });
  ...
  describe("disabled model filtering in by-model", () => {
    ...
    it("includes untracked model (no models row) in by-model breakdown", async () => {
      ...
      expect(utRows.map((r: any) => r.modelId).sort()).toEqual([
        "ghost-m",
        "known-m",
      ]);
    });
    ...
    it("by-model request counts match summary total", async () => {
      ...
      const byPlatform = await request(
        app,
        "/api/analytics/by-platform?range=24h",
      );
      ...
      expect(
        consRows.reduce((sum: number, r: any) => sum + r.requests, 0),
      ).toBe(15);
      ...
      const consPlatform = byPlatform.body.find(
        (r: any) => r.platform === "cons",
      );
  ...
  describe("timeline interval limits", () => {
    ...
    it("coerces minute buckets to daily buckets for 30d timelines", async () => {
      const { status, body } = await request(
        app,
        "/api/analytics/timeline?range=30d&interval=minute",
      );

      expect(status).toBe(200);
      expect(body).toHaveLength(31);
      expect(body.every((pt: any) => pt.timestamp.endsWith("T00:00:00Z"))).toBe(
        true,
      );
    });
    ...
    it("coerces 5-minute buckets to daily buckets for 7d model timelines", async () => {
      const { status, body } = await request(
        app,
        "/api/analytics/model-timeline?range=7d&interval=5min",
      );

      expect(status).toBe(200);
      expect(body.points).toHaveLength(8);
      expect(
        body.points.every((pt: any) => pt.timestamp.endsWith("T00:00:00Z")),
      ).toBe(true);
    });
    ...
    it("still allows hourly buckets for 30d timelines", async () => {
      const { status, body } = await request(
        app,
        "/api/analytics/timeline?range=30d&interval=hour",
      );
  ...
  describe("hourly productivity buckets", () => {
    ...
    it("returns 24 zero-filled buckets when no traffic exists", async () => {
      ...
      const { status, body } = await request(
        app,
        "/api/analytics/hourly?range=24h",
      );
    ...
    it("zero-fills all 24 hours for the 30d range when the window is too coarse", async () => {
      ...
      const { status, body } = await request(
        app,
        "/api/analytics/hourly?range=30d",
      );
  ...
  describe("pings-hourly collapsed baseline", () => {
    ...
    it("returns 24 zero-filled buckets when no pings", async () => {
      ...
      const { status, body } = await request(
        app,
        "/api/analytics/pings-hourly?range=24h",
      );
  ...
  describe("model-timeline stacked series", () => {
    ...
    it("returns top models plus Other with zero-filled timeline points", async () => {
      ...
      const { status, body } = await request(
        app,
        "/api/analytics/model-timeline?range=24h&limit=2",
      );

      expect(status).toBe(200);
      expect(body.series.map((s: any) => s.key)).toEqual([
        "model_0",
        "model_1",
        "other",
      ]);
      expect(body.series.map((s: any) => s.modelId)).toEqual([
        "m1",
        "m2",
        "__other__",
      ]);
  ...
  describe("hourly-by-model per-model baseline", () => {
    ...
    it("returns empty models array when no activity", async () => {
      ...
      const { status, body } = await request(
        app,
        "/api/analytics/hourly-by-model?range=24h",
      );
