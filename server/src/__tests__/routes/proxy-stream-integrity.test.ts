...
const textChunk = (s: string, finish: string | null = null) => ({
  id: "c1",
  object: "chat.completion.chunk",
  created: 1,
  model: "m",
  choices: [{ index: 0, delta: { content: s }, finish_reason: finish }],
});
...
function mockUpstream(script: Array<{ body: string; status?: number }>) {
  ...
  vi.spyOn(global, "fetch").mockImplementation(async (url, init) => {
    ...
    if (
      !/api\.groq\.com|openrouter\.ai|api\.cohere|generativelanguage|integrate\.api\.nvidia|api\.cerebras|api\.mistral|router\.huggingface|api\.cloudflare|models\.github|open\.bigmodel|api\.llm7|api\.kilo|text\.pollinations|ollama\.com|opencode\.ai/.test(
        urlStr,
      )
    ) {
...
describe("proxy stream turn-integrity", () => {
  ...
  it("fails over when the upstream stream opens with an in-band error frame (Groq tool_use_failed)", async () => {
    const up = mockUpstream([
      ...
      },
      {
        body: sse(
          roleChunk,
          textChunk("All good from the next model."),
          finishChunk("stop"),
          "[DONE]",
        ),
      },
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      messages: [{ role: "user", content: "in-band error failover test" }],
    });
    ...
    expect(
      fs.some((f) => f.choices?.[0]?.delta?.content?.includes("All good")),
    ).toBe(true);
  ...
  it("synthesizes finish_reason tool_calls and ids for a stream that ends without a terminal reason", async () => {
    ...
    const tcChunk = (frag: object) => ({
      id: "c1",
      object: "chat.completion.chunk",
      created: 1,
      model: "m",
      choices: [
        { index: 0, delta: { tool_calls: [frag] }, finish_reason: null },
      ],
    });
    mockUpstream([
      {
        body: sse(
          ...
          tcChunk({
            index: 0,
            function: { name: "Read", arguments: '{"file_pa' },
          }),
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      tools: TOOLS,
      messages: [{ role: "user", content: "finish synthesis test" }],
    });
    expect(r.status).toBe(200);
    const fs = frames(r.text);
    ...
    const finishes = fs
      .map((f) => f.choices?.[0]?.finish_reason)
      .filter(Boolean);
  ...
  it("rescues a streamed inline dialect turn into structured tool_calls", async () => {
    mockUpstream([
      {
        body: sse(
          ...
          textChunk(
            'tion_begin|><|tool_call_begin|>functions.Read:0<|tool_call_argument_begin|>{"file_path":"/a"}<|tool_call_end|><|tool_calls_section_end|>',
          ),
          finishChunk("stop"),
          "[DONE]",
        ),
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      tools: TOOLS,
      messages: [{ role: "user", content: "dialect stream rescue test" }],
    });
    expect(r.status).toBe(200);
    const fs = frames(r.text);
    ...
    expect(
      fs.some(
        (f) =>
          ...
          f.choices[0].delta.content.includes("<|tool_call"),
      ),
    ).toBe(false);
    ...
    expect(
      fs.map((f) => f.choices?.[0]?.finish_reason).filter(Boolean),
    ).toEqual(["tool_calls"]);
  });
  ...
  it("fails over an unparseable dialect turn (degraded id token) before headers", async () => {
    const up = mockUpstream([
      {
        body: sse(
          roleChunk,
          textChunk(
            '<|tool_call_begin|> chatcmpl-tool-bde5 <|tool_call_argument_begin|> {"file_path": "/a"}',
          ),
          finishChunk("stop"),
          "[DONE]",
        ),
      },
      {
        body: sse(
          roleChunk,
          textChunk("Recovered by the next model."),
          finishChunk("stop"),
          "[DONE]",
        ),
      },
    ...
    const r = await request(app, "/v1/chat/completions", {
      ...
      tools: TOOLS,
      messages: [
        { role: "user", content: "unparseable dialect failover test" },
      ],
    });
    expect(r.status).toBe(200);
    expect(up.calls()).toBe(2);
    expect(
      frames(r.text).some((f) =>
        f.choices?.[0]?.delta?.content?.includes("Recovered"),
      ),
    ).toBe(true);
  });
  ...
  it("fails over an abrupt EOF that happens before any payload", async () => {
    const up = mockUpstream([
      ...
      },
      {
        body: sse(
          roleChunk,
          textChunk("Second model answers."),
          finishChunk("stop"),
          "[DONE]",
        ),
      },
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      messages: [{ role: "user", content: "pre-payload truncation test" }],
    });
    expect(r.status).toBe(200);
    expect(up.calls()).toBe(2);
    expect(
      frames(r.text).some((f) =>
        f.choices?.[0]?.delta?.content?.includes("Second model"),
      ),
    ).toBe(true);
    const rows = getDb()
      .prepare("SELECT status, error FROM requests ORDER BY id")
  ...
  it("surfaces an honest error frame when truncation happens after payload reached the client", async () => {
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      messages: [{ role: "user", content: "post-payload truncation test" }],
    });
    expect(r.status).toBe(200);
    const fs = frames(r.text);
    expect(
      fs.some((f) => f.choices?.[0]?.delta?.content === "Partial ans"),
    ).toBe(true);
    expect(fs.some((f) => f.error?.type === "stream_error")).toBe(true);
    const rows = getDb()
      .prepare("SELECT status FROM requests ORDER BY id")
      .all() as any[];
  ...
  it("fails over a stream that completes with no content and no tool calls", async () => {
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      messages: [{ role: "user", content: "empty stream failover test" }],
    });
    expect(r.status).toBe(200);
    expect(up.calls()).toBe(2);
    expect(
      frames(r.text).some((f) =>
        f.choices?.[0]?.delta?.content?.includes("Non-empty"),
      ),
    ).toBe(true);
  });
  ...
  it("never leaks raw tool_call deltas riding on role/reasoning chunks (OpenRouter shape)", async () => {
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      tools: TOOLS,
      messages: [{ role: "user", content: "delta leak test" }],
    });
    expect(r.status).toBe(200);
    const fs = frames(r.text);
  ...
  it("emits assembled tool_calls without undefined holes when upstream indexes are sparse", async () => {
    const tcChunk = (frag: object) => ({
      id: "c1",
      object: "chat.completion.chunk",
      created: 1,
    ...
    mockUpstream([
      {
        body: sse(
          ...
          tcChunk({ index: 1, function: { arguments: 'path":"/sparse"}' } }),
          finishChunk("tool_calls"),
          "[DONE]",
        ),
      },
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      tools: TOOLS,
      messages: [{ role: "user", content: "sparse tool-call index test" }],
    });
    expect(r.status).toBe(200);
    const fs = frames(r.text);
    const tcFrame = fs.find((f) => f.choices?.[0]?.delta?.tool_calls);
    expect(tcFrame).toBeDefined();
    const calls = tcFrame.choices[0].delta.tool_calls;
    expect(calls).toHaveLength(1);
    expect(calls[0].id).toBe("sparse_1");
  ...
  it("streams ordinary text through unmodified, always ending in a terminal finish_reason", async () => {
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: true,
      messages: [{ role: "user", content: "plain passthrough test" }],
    });
    const fs = frames(r.text);
  ...
  it("rescues a non-streaming inline dialect answer into structured tool_calls", async () => {
          ...
          choices: [
            {
              ...
              message: {
                role: "assistant",
                content: '<function=Read{"file_path": "/tmp/a"}</function>',
              },
              finish_reason: "stop",
            },
          ],
    ...
    const r = await request(app, "/v1/chat/completions", {
      stream: false,
      tools: TOOLS,
      messages: [{ role: "user", content: "non-stream dialect rescue test" }],
    });
    expect(r.status).toBe(200);
...
describe("sticky session integrity", () => {
  ...
  it("applies to array-of-blocks content (opencode-style agents)", () => {
    const t1 = [
      {
        role: "user" as const,
        content: [{ type: "text", text: "array content sticky probe" }] as any,
      },
    ];
    setStickyModel(t1, 7);
    const t2 = [
      ...
      {
        role: "user" as const,
        content: [{ type: "text", text: "next" }] as any,
      },
    ];
