...
describe("isModelAccessForbiddenError (403 model-not-on-tier, drives whole-model skip — issue #256)", () => {
  it("flags a 403 reaching the proxy by message or attached status", () => {
    ...
    expect(
      isModelAccessForbiddenError(
        new Error(
          "GitHub Models API error 403: Model not available on your plan",
        ),
      ),
    ).toBe(true);
    expect(
      isModelAccessForbiddenError(
        new Error(
          "Cloudflare API error 403: this model requires a subscription",
        ),
      ),
    ).toBe(true);
  ...
  it("does not flag rate limits, 404s, or payment errors", () => {
    expect(
      isModelAccessForbiddenError(new Error("429 Too Many Requests")),
    ).toBe(false);
    expect(
      isModelAccessForbiddenError(
        new Error("OpenRouter API error 404: Provider returned error"),
      ),
    ).toBe(false);
    expect(
      isModelAccessForbiddenError(
        new Error("HuggingFace Router API error 402: Payment required"),
      ),
    ).toBe(false);
  });
});
...
describe("isModelNotFoundError (drives whole-model skip within a request)", () => {
  it("flags 404 / not-found / no-endpoints phrasings", () => {
    ...
    expect(
      isModelNotFoundError(
        new Error(
          "No endpoints found for openrouter/minimax/minimax-m2.5:free",
        ),
      ),
    ).toBe(true);
  });
  ...
  it("does not flag rate limits, 5xx, or payment errors", () => {
    expect(isModelNotFoundError(new Error("429 Too Many Requests"))).toBe(
      false,
    );
    expect(isModelNotFoundError(new Error("503 Service Unavailable"))).toBe(
      false,
    );
    expect(
      isModelNotFoundError(
...
describe("isRetryableError", () => {
  describe("413 Payload Too Large", () => {
    it('treats explicit "413" in the error message as retryable', () => {
      expect(
        isRetryableError(
          new Error("GitHub Models API error 413: Request body too large"),
        ),
      ).toBe(true);
      expect(
        isRetryableError(
          new Error("Cloudflare API error 413: Payload Too Large"),
        ),
      ).toBe(true);
    });
    ...
    it("treats common 413 phrasings (no status code) as retryable", () => {
      ...
      expect(
        isRetryableError(new Error("Request body too large for this model")),
      ).toBe(true);
      expect(isRetryableError(new Error("Request entity too large"))).toBe(
        true,
      );
  ...
  describe("404 model removed / not found (the bug #66 fixes)", () => {
    it('treats explicit "404" in the error message as retryable', () => {
      ...
      expect(
        isRetryableError(new Error("Groq API error 404: model not found")),
      ).toBe(true);
    });
    ...
    it('catches bare "not found" phrasing (any provider, any case)', () => {
      ...
      expect(
        isRetryableError(new Error("The requested model was not found")),
      ).toBe(true);
    });
  });
  ...
  describe("provider tool-call generation 400s fail over (#168)", () => {
    ...
    it('treats any openai-compat "API error 400" as retryable (one provider rejects params another accepts)', () => {
      expect(
        isRetryableError(
          new Error("Cerebras API error 400: tool schema not supported"),
        ),
      ).toBe(true);
    });
  ...
  describe("402 Payment Required out-of-credits fails over (graceful degradation)", () => {
    ...
    it("catches common out-of-credits phrasings", () => {
      ...
      expect(
        isRetryableError(new Error("Insufficient credit for this request")),
      ).toBe(true);
    ...
    it("isPaymentRequiredError flags 402 (drives the long bench) but not a 429", () => {
      ...
      expect(isPaymentRequiredError(new Error("429 Too Many Requests"))).toBe(
        false,
      );
      expect(isPaymentRequiredError(new Error("503 Service Unavailable"))).toBe(
        false,
      );
    });
  });
