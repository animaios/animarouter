...
describe("Vision-aware routing (#118, #125)", () => {
  ...
  it("seeds supports_vision: true for vision models, false for text-only", () => {
    ...
    const vision = db
      .prepare(
        "SELECT supports_vision FROM models WHERE model_id = 'gemini-2.5-flash'",
      )
    ...
    const visionCount = (
      db
        .prepare("SELECT COUNT(*) c FROM models WHERE supports_vision = 1")
        .get() as { c: number }
    ).c;
    const textCount = (
      db
        .prepare("SELECT COUNT(*) c FROM models WHERE supports_vision = 0")
        .get() as { c: number }
    ).c;
  ...
  it("rejects an image request with a clear 422 when no vision model is enabled", async () => {
    ...
    getDb()
      .prepare(
        "UPDATE models SET supports_vision = 1 WHERE model_id IN ('gemini-2.5-flash', 'gemini-2.5-pro', 'gpt-oss-120b')",
      )
      .run();
  });
