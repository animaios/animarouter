...
vi.mock("../../services/router.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../../services/router.js")>();
...
describe("POST /v1/responses (#96)", () => {
  ...
  it("rejects requests without a valid unified key (401)", async () => {
    expect((await post(app, "/v1/responses", { input: "hi" })).status).toBe(
      401,
    );
    expect(
      (await post(app, "/v1/responses", { input: "hi" }, "wrong")).status,
    ).toBe(401);
  });

  it("rejects an invalid body (missing input) with 400", async () => {
    expect(
      (await post(app, "/v1/responses", { model: "auto" }, key)).status,
    ).toBe(400);
  });
  ...
  it("stream: emits the Responses SSE event sequence", async () => {
    ...
    const { status, text, contentType } = await post(
      app,
      "/v1/responses",
