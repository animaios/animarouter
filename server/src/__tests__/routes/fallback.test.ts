...
describe("Fallback API", () => {
  ...
  it("GET /api/fallback excludes archived models from routing views", async () => {
    ...
    try {
      ...
      expect(
        fallback.body.some((entry: any) => entry.modelDbId === target.id),
      ).toBe(false);
      ...
      expect(
        performance.body.some((entry: any) => entry.modelDbId === target.id),
      ).toBe(false);
    } finally {
  ...
  it("persists model boost and returns it in fallback entries", async () => {
    ...
    const boosted = await request(
      app,
      "PUT",
      `/api/fallback/boost/${modelDbId}`,
      { boost: 2 },
    );
    ...
    const boostedEntry = afterBoost.find(
      (entry: any) => entry.modelDbId === modelDbId,
    );
    ...
    const resetEntry = afterReset.find(
      (entry: any) => entry.modelDbId === modelDbId,
    );
  ...
  it("keeps model boost until it is manually reset", async () => {
    ...
    const demotedEntry = afterEviction.find(
      (entry: any) => entry.modelDbId === modelDbId,
    );
  ...
  it("persists group boost across all grouped provider rows", async () => {
    ...
    try {
      const { body: original } = await request(app, "GET", "/api/fallback");
      const group = original.find(
        (entry: any) => entry.isGroup && entry.providers?.length > 0,
      );
      ...
      const boosted = await request(
        app,
        "PUT",
        `/api/fallback/boost/groups/${groupId}`,
        { boost: 2 },
      );
      expect(boosted.status).toBe(200);
      expect(boosted.body).toMatchObject({ groupId, boost: 2 });
      expect(
        boosted.body.modelDbIds.sort((a: number, b: number) => a - b),
      ).toEqual([...providerIds].sort((a, b) => a - b));

      const { body: afterBoost } = await request(app, "GET", "/api/fallback");
      const boostedGroup = afterBoost.find(
        (entry: any) => entry.groupId === groupId,
      );
      ...
      for (const providerId of providerIds) {
        const row = db
          .prepare("SELECT boost FROM model_degradation WHERE model_db_id = ?")
          .get(providerId) as { boost: number } | undefined;
      ...
      const { body: afterReset } = await request(app, "GET", "/api/fallback");
      const resetGroup = afterReset.find(
        (entry: any) => entry.groupId === groupId,
      );
