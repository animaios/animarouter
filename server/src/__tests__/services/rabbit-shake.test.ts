...
describe("Rabbit Shake routing helpers", () => {
  ...
  it("orders DB-backed foundation candidates by Rabbit score and requires an enabled key", () => {
    ...
    expect(candidates[0].rabbitScore).toBeGreaterThan(candidates[1].rabbitScore);
  });

  it("resolves foundation candidates by auto, top-rank, and explicit override", () => {
    const candidates = [
      candidate({ modelDbId: 10, modelId: "auto-first", intelligenceRank: 4, rabbitScore: 0.9 }),
      candidate({ modelDbId: 20, modelId: "rank-first", intelligenceRank: 1, rabbitScore: 0.7 }),
      candidate({ modelDbId: 30, modelId: "explicit", intelligenceRank: 2, rabbitScore: 0.5 }),
    ];

    expect(resolveFoundationCandidates(config(), candidates).map((item) => item.modelDbId)).toEqual([10, 20, 30]);
    expect(resolveFoundationCandidates(config({ foundationSelection: "top_rank" }), candidates).map((item) => item.modelDbId)).toEqual([20, 30, 10]);
    expect(resolveFoundationCandidates(config({ foundationSelection: 30 }), candidates).map((item) => item.modelDbId)).toEqual([30, 10, 20]);
  });

  it("resolves injection model without hardcoded providers or model names", () => {
    const candidates = [
      candidate({ modelDbId: 1, platform: "alpha", modelId: "foundation", sizeLabel: "Frontier", intelligenceRank: 1, rabbitScore: 0.95 }),
      candidate({ modelDbId: 2, platform: "alpha", modelId: "same-provider", sizeLabel: "Frontier", intelligenceRank: 2, rabbitScore: 0.9 }),
      candidate({ modelDbId: 3, platform: "beta", modelId: "divergent", sizeLabel: "Large", intelligenceRank: 3, rabbitScore: 0.85 }),
      candidate({ modelDbId: 4, platform: "gamma", modelId: "top-rank", sizeLabel: "Frontier", intelligenceRank: 1, rabbitScore: 0.7 }),
    ];

    expect(resolveInjectionModel(config(), 1, candidates)?.modelDbId).toBe(3);
    expect(resolveInjectionModel(config({ injectionSelection: "top_rank" }), 1, candidates)?.modelDbId).toBe(4);
    expect(resolveInjectionModel(config({ injectionSelection: "different_tier" }), 1, candidates)?.modelDbId).toBe(3);
    expect(resolveInjectionModel(config({ injectionSelection: 2 }), 1, candidates)?.modelDbId).toBe(2);
  });
