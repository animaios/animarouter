...
function addGroup(opts: {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
  `).run(
    opts.groupKey, opts.displayName, opts.intelligenceRank, opts.sizeLabel,
  ...
  const groupId = (db.prepare('SELECT id FROM model_groups WHERE group_key = ?')
    .get(opts.groupKey) as { id: number }).id;
  ...
  for (const p of opts.providers) {
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, '', 1, ?)
    `).run(
      p.platform, p.modelId, p.name,
      p.intelligenceRank ?? opts.intelligenceRank,
      p.speedRank,
      p.sizeLabel ?? opts.sizeLabel,
      groupId,
    );
    const mId = (db.prepare('SELECT id FROM models WHERE platform = ? AND model_id = ?')
      .get(p.platform, p.modelId) as { id: number }).id;
    modelIds.push(mId);
  ...
  db.prepare('INSERT INTO fallback_config (model_db_id, priority, group_id, enabled) VALUES (?, ?, ?, 1)')
  ...
  for (const mId of modelIds) {
    db.prepare('UPDATE fallback_config SET enabled = 0 WHERE model_db_id = ? AND group_id IS NULL').run(mId);
  }
...
describe('group-aware routing', () => {
  ...
  it('when grouping is off, router uses the classic model_db_id chain', () => {
    addModel({ platform: 'google', modelId: 'gemini', name: 'Gemini', intelligenceRank: 1, sizeLabel: 'Frontier', budget: '~10M', priority: 1 });
  ...
  it('routes to a provider within a single group (priority mode)', () => {
    addGroup({
      groupKey: 'gpt-4o', displayName: 'GPT-4o',
      intelligenceRank: 1, sizeLabel: 'Frontier',
      priority: 1,
      providers: [
        { platform: 'nim', modelId: 'nim/gpt-4o', name: 'GPT-4o (NIM)', speedRank: 5 },
        { platform: 'zen', modelId: 'zen/gpt-4o', name: 'GPT-4o (Zen)', speedRank: 10 },
      ],
    });
    setSetting('model_grouping_enabled', 'true');
    setRoutingStrategy('priority');
  ...
  it('routes to a provider within a single group (balanced mode)', () => {
    addGroup({
      groupKey: 'deepseek-v4-flash', displayName: 'DeepSeek V4 Flash',
      intelligenceRank: 3, sizeLabel: 'Large',
      priority: 1,
      providers: [
        { platform: 'nim', modelId: 'nim/dsv4f', name: 'DSV4F (NIM)', speedRank: 3 },
        { platform: 'zen', modelId: 'zen/dsv4f', name: 'DSV4F (Zen)', speedRank: 8 },
      ],
    });
    setSetting('model_grouping_enabled', 'true');
  ...
  it('falls through to the next group when all providers in first group are exhausted', () => {
    ...
    addGroup({
      groupKey: 'smart-model', displayName: 'Smart Model',
      intelligenceRank: 1, sizeLabel: 'Frontier',
      priority: 1,
      providers: [
        { platform: 'smart-provider', modelId: 'smart/m1', name: 'M1', speedRank: 1 },
      ],
    });
    ...
    addGroup({
      groupKey: 'backup-model', displayName: 'Backup Model',
      intelligenceRank: 5, sizeLabel: 'Medium',
      priority: 2,
      providers: [
        { platform: 'backup-provider', modelId: 'backup/m2', name: 'M2', speedRank: 5 },
      ],
    });
  ...
  it('skips a non-vision group when requireVision is set', () => {
    const db = getDb();
    addGroup({
      groupKey: 'text-only', displayName: 'Text Only',
      intelligenceRank: 1, sizeLabel: 'Frontier',
      priority: 1,
    ...
    addGroup({
      groupKey: 'vision-model', displayName: 'Vision Model',
      intelligenceRank: 3, sizeLabel: 'Large',
      priority: 2,
      supportsVision: true,
      providers: [
        { platform: 'prov-b', modelId: 'b/vision', name: 'Vision', speedRank: 5 },
      ],
    });
    setSetting('model_grouping_enabled', 'true');
    setRoutingStrategy('priority');
  ...
  it('skips a non-tools group when requireTools is set', () => {
    addGroup({
      groupKey: 'no-tools', displayName: 'No Tools',
      intelligenceRank: 1, sizeLabel: 'Frontier',
      priority: 1,
      supportsTools: false,
      providers: [
        { platform: 'prov-a', modelId: 'a/notools', name: 'NoTools', speedRank: 1 },
      ],
    });
    addGroup({
      groupKey: 'tools-model', displayName: 'Tools Model',
      intelligenceRank: 3, sizeLabel: 'Large',
      priority: 2,
      supportsTools: true,
      providers: [
  ...
  it('skips a group whose context_window is too small', () => {
    addGroup({
      groupKey: 'small-ctx', displayName: 'Small Context',
      intelligenceRank: 1, sizeLabel: 'Frontier',
      priority: 1,
      contextWindow: 1000,
      providers: [
    ...
    addGroup({
      groupKey: 'big-ctx', displayName: 'Big Context',
      intelligenceRank: 3, sizeLabel: 'Large',
      priority: 2,
  ...
  it('getRoutingScores returns groupedScores when grouping is on', () => {
    ...
    if (g.providers.length >= 2) {
      expect(g.providers[0].subScore).toBeGreaterThanOrEqual(g.providers[1].subScore);
    }
  });

  it('getRoutingScores omits groupedScores when grouping is off', () => {
    addModel({ platform: 'google', modelId: 'm1', name: 'M1', intelligenceRank: 3, sizeLabel: 'Large', budget: '~50M', priority: 1 });
    setRoutingStrategy('balanced');
    refreshStatsCache(getDb(), true);
    const result = getRoutingScores();
  ...
  it('disabled groups are skipped in the fallback chain', () => {
    const db = getDb();
    addGroup({
      groupKey: 'disabled-group', displayName: 'Disabled',
      intelligenceRank: 1, sizeLabel: 'Frontier',
      priority: 1,
      providers: [
    ...
    // Disable the group
    db.prepare('UPDATE model_groups SET enabled = 0 WHERE group_key = ?').run('disabled-group');

    addGroup({
      groupKey: 'active-group', displayName: 'Active',
      intelligenceRank: 5, sizeLabel: 'Medium',
      priority: 2,
      providers: [
  ...
  it('disabled providers within a group are skipped', () => {
    const db = getDb();
    addGroup({
      groupKey: 'test-group', displayName: 'Test',
      intelligenceRank: 1, sizeLabel: 'Frontier',
      priority: 1,
      providers: [
    ...
    // Disable provider A
    db.prepare("UPDATE models SET enabled = 0 WHERE platform = 'prov-a' AND model_id = 'a/m'").run();
  ...
  it('balanced strategy sorts groups by representative score (intelligence wins)', () => {
    // Frontier group (smarter) should beat Medium group even if Medium has priority=1
    addGroup({
      groupKey: 'medium-group', displayName: 'Medium',
      intelligenceRank: 8, sizeLabel: 'Medium',
      priority: 1,
      providers: [
        { platform: 'prov-a', modelId: 'a/med', name: 'Med', speedRank: 1 },
      ],
    });
    addGroup({
      groupKey: 'frontier-group', displayName: 'Frontier',
      intelligenceRank: 1, sizeLabel: 'Frontier',
      priority: 2,
      providers: [
        { platform: 'prov-b', modelId: 'b/front', name: 'Front', speedRank: 10 },
      ],
    });
    setSetting('model_grouping_enabled', 'true');
