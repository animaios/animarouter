...
describe("bandit router", () => {
  ...
  it("smartest vs fastest flips which model wins, at equal reliability", () => {
    ...
    for (const s of dbg.scores)
      console.log(
        `fastest ${s.modelId}: spd=${s.speed.toFixed(4)} int=${s.intelligence.toFixed(4)} rel=${s.reliability.toFixed(4)} score=${s.score.toFixed(4)} reqs=${s.totalRequests}`,
      );
  ...
  it("custom weights persist normalized; default to balanced until saved", () => {
    expect(getCustomWeights()).toEqual({
      reliability: 0.4,
      speed: 0.2,
      intelligence: 0.2,
      latency: 0.2,
