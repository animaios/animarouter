...
describe("classifyError", () => {
  it("returns minor for 429 via status field", () => {
    expect(classifyError({ status: 429, message: "rate limited" })).toBe(
      "minor",
    );
  });
  ...
  it('returns null for 429 with "quota" or "insufficient" in message (hard quota)', () => {
    expect(
      classifyError({ status: 429, message: "quota exceeded" }),
    ).toBeNull();
    expect(
      classifyError({ status: 429, message: "insufficient credits" }),
    ).toBeNull();
  });
...
describe("getDegradationFactor", () => {
  ...
  it("at penalty=100 → factor ≈ 0.020 (within 0.005)", () => {
    ...
    expect(getDegradationFactor(1)).toBeCloseTo(0.02, 3);
  });
