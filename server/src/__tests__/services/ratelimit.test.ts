...
describe("Rate Limiter", () => {
  ...
  describe("canMakeRequest", () => {
    it("should allow request when under RPM limit", () => {
      expect(
        canMakeRequest("groq", "llama-70b", testId, {
          rpm: 30,
          rpd: null,
          tpm: null,
          tpd: null,
    ...
    it("should allow request when limits are null (unlimited)", () => {
      expect(
        canMakeRequest("nvidia", "nemotron", testId, {
          rpm: null,
          rpd: null,
          tpm: null,
          tpd: null,
  ...
  describe("canUseTokens", () => {
    it("should allow tokens when under TPM limit", () => {
      expect(
        canUseTokens("groq", "llama-70b", testId, 500, {
          tpm: 6000,
          tpd: null,
        }),
      ).toBe(true);
    ...
    it("should deny tokens when TPM limit would be exceeded", () => {
      ...
      expect(
        canUseTokens("cerebras", "qwen3", testId, 20000, {
          tpm: 60000,
          tpd: null,
        }),
      ).toBe(false);
    ...
    it("should allow when limit is null", () => {
      expect(
        canUseTokens("nvidia", "nemotron", testId, 100000, {
          tpm: null,
          tpd: null,
        }),
      ).toBe(true);
  ...
  describe("escalating cooldown", () => {
    ...
    it("counts independently per (platform, model, key)", () => {
      ...
      expect(getNextCooldownDuration("groq", `m-${id}`, id)).toBe(
        2 * 60 * 1000,
      );
      expect(getNextCooldownDuration("groq", `m-${id}`, id + 1)).toBe(
        2 * 60 * 1000,
      );
      expect(getNextCooldownDuration("groq", `m-${id}-other`, id)).toBe(
        2 * 60 * 1000,
      );
    });
  });
  ...
  describe("getCooldownDurationForLimit (daily vs transient 429)", () => {
    it("uses a short, non-escalating cooldown when the daily quota is NOT exhausted", () => {
      ...
      expect(
        getCooldownDurationForLimit(...args, { rpd: 1000, tpd: null }),
      ).toBe(90 * 1000);
      expect(
        getCooldownDurationForLimit(...args, { rpd: 1000, tpd: null }),
      ).toBe(90 * 1000);
    ...
    it("treats a null daily limit as never-exhausted (always transient)", () => {
      const id = Math.floor(Math.random() * 1_000_000);
      for (let i = 0; i < 50; i++)
        recordRequest("mistral", `nolimit-${id}`, id);
      expect(
        getCooldownDurationForLimit("mistral", `nolimit-${id}`, id, {
          rpd: null,
          tpd: null,
        }),
      ).toBe(90 * 1000);
    ...
    it("escalates only once the daily request limit is actually reached", () => {
      ...
      expect(
        getCooldownDurationForLimit(platform, model, id, { rpd: 5, tpd: null }),
      ).toBe(90 * 1000);
  ...
  describe("persistent state", () => {
    it("preserves per-key usage and cooldowns after the limiter module reloads", async () => {
      ...
      const dbPath = `/tmp/animarouter-ratelimit-${Date.now()}-${Math.random()}.db`;
      const keyId = 4242;
      ...
      try {
        ...
        expect(
          limiterAfterReload.canMakeRequest("groq", "persistent-model", keyId, {
            rpm: null,
            rpd: 1,
            tpm: null,
            tpd: null,
        ...
        expect(
          limiterAfterReload.canUseTokens(
            "groq",
            "persistent-model",
            keyId,
            100,
        ...
        expect(
          limiterAfterReload.isOnCooldown("groq", "persistent-model", keyId),
        ).toBe(true);
      } finally {
        db?.close();
        removeDbFile(dbPath);
...
describe("Cooldown duration with upstream Retry-After", () => {
  ...
  it("uses the transient cooldown when no Retry-After is given", () => {
    expect(getCooldownDurationForLimit("groq", "m", testId, noLimits)).toBe(
      90_000,
    );
  });
  ...
  it("never benches shorter than the heuristic (ignores a shorter Retry-After)", () => {
    expect(
      getCooldownDurationForLimit("groq", "m", testId, noLimits, 30_000),
    ).toBe(90_000);
  });

  it("honors a Retry-After longer than the heuristic", () => {
    expect(
      getCooldownDurationForLimit("groq", "m", testId, noLimits, 300_000),
    ).toBe(300_000);
  });
...
describe("getCooldownDurationForLimit with heartbeat", () => {
  ...
  it("uses normal transient cooldown when heartbeat enabled", async () => {
    ...
    expect(
      mod.getCooldownDurationForLimit("groq", "model", testId, {
        rpd: null,
        tpd: null,
      }),
    ).toBe(90_000);
