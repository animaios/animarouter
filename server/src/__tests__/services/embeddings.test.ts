...
describe("embeddings service", () => {
  ...
  describe("migration seed", () => {
    it("seeds the embedding catalog with families and a default", () => {
      const rows = getDb()
        .prepare("SELECT DISTINCT family FROM embedding_models")
    ...
    it("cohere is seeded disabled (its quota is shared with chat)", () => {
      const row = getDb()
        .prepare(
          "SELECT enabled FROM embedding_models WHERE platform = 'cohere'",
        )
  ...
  describe("resolveFamily", () => {
    ...
    it("maps a provider-specific model id to its family", () => {
      ...
      expect(resolveFamily("nvidia/llama-nemotron-embed-vl-1b-v2")).toBe(
  ...
  describe("runEmbeddings", () => {
    it("rejects unknown models with a 400", async () => {
      await expect(
        runEmbeddings("no-such-model", ["hi"]),
    ...
    it("embeds via the first provider in the family chain", async () => {
      ...
      const result = await runEmbeddings("llama-nemotron-embed-vl-1b-v2", [
        "hello",
      ]);
    ...
    it("throws 429 when every provider is rate-limited", async () => {
      ...
      await expect(
        runEmbeddings("llama-nemotron-embed-vl-1b-v2", ["hello"]),
    ...
    it("throws 502 when the family has no usable providers", async () => {
      ...
      getDb()
        .prepare(
          "DELETE FROM api_keys WHERE platform IN (SELECT DISTINCT platform FROM embedding_models WHERE family = 'bge-m3')",
        )
        .run();
      await expect(runEmbeddings("bge-m3", ["hello"])).rejects.toMatchObject({
        status: 502,
      });
    });
    ...
    it("splits cloudflare account_id:token keys", async () => {
      ...
      expect(String(fetchMock.mock.calls[0][0])).toContain(
        "/accounts/acct-123/ai/v1/embeddings",
      );
