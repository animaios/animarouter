...
describe("inline tool-call dialect rescue", () => {
  describe("Kimi/DeepSeek token dialect", () => {
    it("parses a well-formed functions.NAME:IDX call", () => {
      const text =
        '<|tool_calls_section_begin|><|tool_call_begin|>functions.Read:0<|tool_call_argument_begin|>{"file_path":"/tmp/x.txt"}<|tool_call_end|><|tool_calls_section_end|>';
    ...
    it("reports detected-but-unparseable when the id token is degraded (no function name)", () => {
      ...
      const text =
        '<|tool_calls_section_begin|> <|tool_call_begin|> chatcmpl-tool-bde5fae954a11b1b <|tool_call_argument_begin|> {"file_path": "/private/tmp/demo-project/src"} <|tool_call_end|>';
      const r = rescueInlineToolCalls(text, TOOLS);
      expect(r.detected).toBe(true);
    ...
    it("rejects a call naming a tool the request never declared", () => {
      const text =
        "<|tool_call_begin|>functions.DropTables:0<|tool_call_argument_begin|>{}<|tool_call_end|>";
      const r = rescueInlineToolCalls(text, TOOLS);
      expect(r.detected).toBe(true);
      expect(r.calls).toBeNull();
    });
  ...
  describe("llama/groq <function=> dialect", () => {
    ...
    it("treats array-shaped arguments as unparseable (live Groq failure shape)", () => {
      const text =
        '<function=Bash [{"a": "npm run build"}, {"b": "/tmp"}]</function>';
      const r = rescueInlineToolCalls(text, TOOLS);
      expect(r.detected).toBe(true);
      expect(r.calls).toBeNull();
    });
  ...
  describe("Qwen/Hermes <tool_call> XML dialect", () => {
    ...
    it('accepts the "parameters" key and multiple blocks', () => {
      const text =
        '<tool_call>{"name": "Read", "parameters": {"file_path": "/a"}}</tool_call><tool_call>{"name": "Grep", "arguments": {"pattern": "x"}}</tool_call>';
      const r = rescueInlineToolCalls(text, TOOLS);
      expect(r.calls).toHaveLength(2);
    });
  ...
  describe("bare / fenced JSON dialect (schema-gated)", () => {
    ...
    it("does NOT touch plain prose", () => {
      const r = rescueInlineToolCalls(
        "The port is 49152, configured in config.json.",
        TOOLS,
      );
      expect(r.detected).toBe(false);
    });
  ...
  describe("stream hold-window helpers", () => {
    it("startsWithDialectMarker matches all dialects, with leading whitespace", () => {
      expect(startsWithDialectMarker("<|tool_calls_section_begin|>x")).toBe(
        true,
      );
    ...
    it("containsDialectMarker finds markers mid-text", () => {
      expect(
        containsDialectMarker('prose then <function=Read{"a":1}</function>'),
      ).toBe(true);
