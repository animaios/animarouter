...
describe("GoogleProvider", () => {
  ...
  it("should translate OpenAI tools/tool_choice to Gemini tools/toolConfig", async () => {
    ...
    expect(capturedBody.tools[0].functionDeclarations[0].name).toBe(
      "get_weather",
    );
    expect(capturedBody.toolConfig.functionCallingConfig.mode).toBe("ANY");
    expect(
      capturedBody.toolConfig.functionCallingConfig.allowedFunctionNames,
    ).toEqual(["get_weather"]);
  });
  ...
  it("should preserve and pass through thought_signature", async () => {
    ...
    expect(result.choices[0].message.tool_calls?.[0].thought_signature).toBe(
      "sig_123",
    );
    ...
    const assistantEntry = capturedBody.contents.find(
      (c: any) => c.role === "model",
    );
