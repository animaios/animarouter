...
function seed() {
  ...
  PROFILES.forEach((p, i) => {
    ...
    db.prepare(
      `INSERT INTO api_keys (platform, label, encrypted_key, iv, auth_tag, status, enabled) VALUES (?, 'sim', ?, ?, ?, 'healthy', 1)`,
...
function main() {
  ...
  for (const p of PROFILES) {
    ...
    console.log(
      `    ${p.name.padEnd(22)} ${p.sizeLabel.padEnd(9)} success ${rate}%  ${tps} tok/s  ${p.ttfbMs}ms ttfb`,
    );
  }
  ...
  `);
  for (let i = 0; i < 300; i++)
    insHist.run(favProfile.platform, favProfile.modelId);
  console.log(
