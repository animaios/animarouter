...
export const DEFAULT_MODEL_GROUP_ALIASES: Array<{
  alias: string;
  groupKey: string;
}> = [
  ...
  { alias: "minimax-m3-free", groupKey: "minimaxai/minimax-m3" },
  {
    ...
    groupKey: "nvidia/nemotron-3-ultra-550b-a55b",
  },
  {
    alias: "nvidia/nemotron-3-ultra-550b-a55b:free",
    groupKey: "nvidia/nemotron-3-ultra-550b-a55b",
  },
];

export const DEFAULT_MODEL_GROUP_DISPLAY_NAMES: Array<{
  groupKey: string;
  displayName: string;
...
export function loadAliasCache(db: Database.Database): Map<string, string> {
  const rows = db
    .prepare("SELECT alias, group_key FROM model_group_aliases")
    .all() as Array<{ alias: string; group_key: string }>;
  ...
  const tx = db.transaction(() => {
    for (const item of DEFAULT_MODEL_GROUP_ALIASES) {
      const result = insert.run(
        normalizeGroupAlias(item.alias),
...
export function applyDefaultModelGroupDisplayNames(
...
export function propagateGroupProperties(
  db: Database.Database,
  groupId: number,
): void {
  const group = db
  ...
  console.log(
    `[ModelGroups] Propagated group ${group.group_key} (id=${groupId}) to ${result.changes} model rows`,
  );
}

export function propagateAllGroupProperties(db: Database.Database): void {
  const groups = db.prepare("SELECT id FROM model_groups").all() as Array<{
    id: number;
  }>;
...
export function reconcileGroups(db: Database.Database): {
  groupsCreated: number;
  modelsReassigned: number;
} {
  const cache = getAliasCache(db);

  const models = db
    .prepare("SELECT id, model_id, group_id FROM models")
    .all() as Array<{ id: number; model_id: string; group_id: number | null }>;

  const findGroupByKey = db.prepare(
    "SELECT id FROM model_groups WHERE group_key = ?",
  );
  const insertGroup = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const setModelGroupId = db.prepare(
  ...
  const tx = db.transaction(() => {
    for (const model of models) {
      ...
      const existingGroup = findGroupByKey.get(groupKey) as
        | { id: number }
        | undefined;
      ...
      if (model.group_id === null) {
        ...
        const currentGroup = db
          .prepare("SELECT group_key FROM model_groups WHERE id = ?")
          .get(model.group_id) as { group_key: string } | undefined;
  ...
  if (groupsCreated > 0 || modelsReassigned > 0) {
    console.log(
      `[ModelGroups] Reconcile: ${groupsCreated} groups created, ${modelsReassigned} models reassigned`,
    );
  }
...
function mergeGroupProperties(db: Database.Database, groupId: number): void {
  const group = db
    .prepare("SELECT group_key FROM model_groups WHERE id = ?")
    .get(groupId) as { group_key: string } | undefined;
  if (!group) return;
  ...
  const benchmarkScore = members.reduce(
    (best: any, m: any) =>
      (m.benchmark_score ?? -1) > (best.benchmark_score ?? -1) ? m : best,
    members[0],
  ).benchmark_score;
  const intelligenceRank = members.reduce(
    (best: any, m: any) =>
      (m.intelligence_rank ?? 0) > (best.intelligence_rank ?? 0) ? m : best,
    members[0],
  ).intelligence_rank;
  const sizeLabel = members[0].size_label;
  const contextWindow = members.reduce(
    (max: number | null, m: any) => {
      if (m.context_window == null) return max;
      return max == null ? m.context_window : Math.max(max, m.context_window);
    },
    null as number | null,
  );
  const maxOutputTokens = members.reduce(
    (max: number | null, m: any) => {
      if (m.max_output_tokens == null) return max;
      return max == null
        ? m.max_output_tokens
        : Math.max(max, m.max_output_tokens);
    },
    null as number | null,
  );
...
export function ensureModelInGroup(
  db: Database.Database,
  modelId: number,
): number {
  const cache = getAliasCache(db);
  const model = db
    .prepare("SELECT id, model_id, group_id FROM models WHERE id = ?")
    .get(modelId) as
    | { id: number; model_id: string; group_id: number | null }
    | undefined;
  ...
  db.prepare("UPDATE models SET group_id = ? WHERE id = ?").run(
    groupId,
    model.id,
  );
