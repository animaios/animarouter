...
keysRouter.post("/", (req: Request, res: Response) => {
  ...
  if (isKeyless) {
    ...
    if (existing) {
      db.prepare(
        "UPDATE api_keys SET label = ?, enabled = 1, status = 'unknown', use_proxy = ? WHERE id = ?",
  ...
  pokeKey(Number(result.lastInsertRowid)).catch((e) =>
    console.warn("[Keys] pokeKey failed:", e),
  );
});
