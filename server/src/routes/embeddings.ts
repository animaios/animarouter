...
embeddingsRouter.put("/", (req: Request, res: Response) => {
  ...
  if (parsed.data.defaultFamily) {
    const exists = db
      .prepare("SELECT 1 FROM embedding_models WHERE family = ?")
      .get(parsed.data.defaultFamily);
    if (!exists) {
      res.status(400).json({
        error: { message: `Unknown family '${parsed.data.defaultFamily}'` },
      });
      return;
    }
  ...
  if (parsed.data.providers) {
    const update = db.prepare(
      "UPDATE embedding_models SET priority = ? WHERE id = ?",
    );
