...
import { isHeartbeatEnabled, markKeyUnhealthy } from "../services/heartbeat.js";
import {
  ...
  for (const item of req.input) {
    ...
    if (
      content.some((p) => {
        ...
        return (
          type === "input_image" || type === "image_url" || type === "image"
        );
      })
    )
      return true;
...
/**
 * Build the streaming skeleton headers + created/in_progress events for the
 * Responses API. Idempotent — only the first call sends anything.
 */
function sendResponsesStreamSkeleton(opts: {
  res: Response;
  sse: (event: string, payload: Record<string, unknown>) => void;
  responseId: string;
  model: string;
  platform: string;
  modelId: string;
  streamStarted: { value: boolean };
  attempt: number;
}): void {
  const {
    res,
  ...
  res.setHeader("X-Routed-Via", `${platform}/${modelId}`);
...
/**
 * Stream the Responses-api-shaped SSE events for one attempt. Mutates the
 * shared skipKeys/lastError/counters on retryable failures.
 * Returns true if the response was fully served and the outer loop should return.
 */
async function handleResponsesStream(opts: {
  res: Response;
  sse: (event: string, payload: Record<string, unknown>) => void;
  route: RouteResult;
  ...
  >();
  let dialMode: "undecided" | "passthrough" | "dialect" = "undecided";
  let heldText = "";

  const openTextItem = (text: string) => {
    const localMsgItemId = msgItemId ?? newId("msg");
    if (msgItemId === null) {
      msgItemId = localMsgItemId;
  ...
  let accCapturedOutputTokens = 0;
  let accCapturedReasoningTokens = 0;
  ...
  try {
    ...
    for await (const chunk of gen) {
      ...
      if (anyChunk.error && !anyChunk.choices) {
        throw new Error(
          `in-band provider error from ${route.displayName}: ${anyChunk.error.message ?? "provider error"}`,
        );
      }
      sendResponsesStreamSkeleton({
        res,
        sse,
        responseId,
        model: route.modelId,
      ...
      if (reasoningText.length > 0) {
        accCapturedReasoningTokens += Math.ceil(reasoningText.length / 4);
      }
      if (text) {
        accCapturedOutputTokens += Math.ceil(text.length / 4);
        if (dialMode === "passthrough") {
          ...
          heldText += text;
          if (dialMode === "undecided") {
            const probe = heldText.trimStart();
            if (startsWithDialectMarker(probe)) {
              dialMode = "dialect";
            } else if (
              !couldBecomeDialectMarker(probe) ||
              heldText.length > 256
            ) {
              dialMode = "passthrough";
        ...
        if (!acc) {
          if (msgItemId !== null && msgText.length > 0) {
            ...
            sse("response.content_part.done", {
              ...
              output_index: 0,
              content_index: 0,
              part: { type: "output_text", text: msgText, annotations: [] },
            });
            sse("response.output_item.done", {
              ...
              item: {
                ...
                role: "assistant",
                content: [
                  { type: "output_text", text: msgText, annotations: [] },
                ],
              },
            });
            msgItemId = null;
  ...
  } catch (streamErrInLoop: any) {
    if (streamStarted.value) {
      const safeError = sanitizeProviderErrorMessage(streamErrInLoop.message);
      try {
        sse("response.failed", {
          response: {
            id: responseId,
    ...
    throw streamErrInLoop;
  }

  const toolSet = new Set((tools ?? []).map((t) => t.function.name));
  if (heldText.length > 0) {
    const rescue =
      dialMode === "dialect" || containsDialectMarker(heldText)
        ? rescueInlineToolCalls(heldText, toolSet)
    ...
    if (rescue.detected && !rescue.calls) {
      logRequest(
        ...
        0,
        Date.now() - start,
        `unparseable inline tool-call dialect: ${heldText.slice(0, 120)}`,
      );
      ...
      if (isHeartbeatEnabled()) {
        markKeyUnhealthy(
          route.keyId,
          route.modelId,
          "model skip: unparseable dialect",
          true,
        );
      }
      lastError.value = new Error(
        `unparseable inline tool-call dialect from ${route.displayName}`,
      );
      return false;
    }
    if (rescue.detected && rescue.calls) {
      console.log(
        `[Responses] Rescued ${rescue.calls.length} inline tool call(s) from ${route.displayName}`,
      );
  ...
  if (msgText.length === 0 && toolAcc.size === 0) {
    ...
    if (isHeartbeatEnabled()) {
      markKeyUnhealthy(
        route.keyId,
        route.modelId,
        "model skip: empty completion",
        true,
      );
    }
    lastError.value = new Error(`empty completion from ${route.displayName}`);
    return false; // retry
  }
  ...
  if (msgItemId !== null) {
    ...
    sse("response.output_item.done", {
      ...
      item: {
        ...
        status: "completed",
        role: "assistant",
        content: [{ type: "output_text", text: msgText, annotations: [] }],
      },
    });
  }
  ...
  for (const acc of toolAcc.values()) {
    ...
    finalToolCalls.push({
      id: acc.callId,
      type: "function",
      function: { name: acc.name, arguments: repairedArgs },
    });
  }
  ...
  const finalResponse = buildResponseObject({
    ...
    completionTokens: accCapturedOutputTokens,
  });
  ...
  recordTokens(
    ...
    route.modelId,
    route.keyId,
    estimatedInputTokens + accCapturedOutputTokens,
  );
  ...
  logRequest(
    ...
    "success",
    estimatedInputTokens,
    accCapturedOutputTokens,
    Date.now() - start,
    null,
    null,
    null,
    accCapturedReasoningTokens,
  );
  return true;
}

async function handleResponsesCompletion(opts: {
  route: RouteResult;
  messages: ChatMessage[];
  tools: ChatToolDefinition[] | undefined;
  toolSchemas: Map<string, any>;
  ...
  let toolCalls = (msg?.tool_calls ?? []).map((tc) => ({
    ...
    function: {
      ...
      arguments: repairToolArguments(
        ...
        toolSchemas.get(tc.function.name),
      ),
    },
  }));
  ...
  if (wantsTools && toolCalls.length === 0 && text) {
    const toolSet = new Set((tools ?? []).map((t) => t.function.name));
    const rescue = rescueInlineToolCalls(text, toolSet);
    if (rescue.detected) {
      if (!rescue.calls) {
        throw new Error(
          `unparseable inline tool-call dialect from ${route.displayName}: ${text.slice(0, 120)}`,
        );
      }
      console.log(
        `[Responses] Rescued ${rescue.calls.length} inline tool call(s) from ${route.displayName}`,
      ...
      toolCalls = rescue.calls.map((c, i) => ({
        ...
        function: {
          name: c.name,
          arguments: repairToolArguments(c.arguments, toolSchemas.get(c.name)),
        },
      }));
  ...
  if (!text && toolCalls.length === 0) {
    ...
    lastError.value = new Error(`empty completion from ${route.displayName}`);
    return false; // retry (loop continues)
  }
...
async function handleResponsesError(opts: {
  err: any;
  route: RouteResult;
  stream: boolean;
  streamStarted: { value: boolean };
...
responsesRouter.post("/responses", async (req: Request, res: Response) => {
  ...
  if (responsesInputHasImage(reqData)) {
    res.status(422).json({
      error: {
        message:
          "Image input is not yet supported on /v1/responses. Use /v1/chat/completions with an image_url content part instead.",
        type: "invalid_request_error",
  ...
  const estimatedTotal =
    estimatedInputTokens + (reqData.max_output_tokens ?? 1000);
  ...
  const lastError: { value: any } = { value: null };
  const streamStarted: { value: boolean } = { value: false };
  let seq = 0;
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      const status = lastError.value ? 429 : (err.status ?? 503);
      const message = lastError.value
        ? `All models exhausted. Last error: ${sanitizeProviderErrorMessage(lastError.value.message)}`
        : err.message;
      const type = lastError.value ? "rate_limit_error" : "routing_error";
      if (streamStarted.value) {
        sse("response.failed", {
          response: {
            id: responseId,
    ...
    try {
      if (stream) {
        const done = await handleResponsesStream({
          res,
          sse,
          route,
          messages,
        ...
        const done = await handleResponsesCompletion({
          route,
          messages,
          tools,
          toolSchemas,
      ...
      const result = await handleResponsesError({
        err,
        route,
        stream,
        streamStarted,
  ...
  const exhaustedMsg = `All models exhausted after ${maxRetries} attempts. Last: ${lastError.value?.message}`;
  if (streamStarted.value) {
    sse("response.failed", {
      response: {
        id: responseId,
