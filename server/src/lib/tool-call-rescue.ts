...
function callFromNamedJson(
  json: string,
  toolNames: Set<string>,
): RescuedToolCall | null {
  let obj: unknown;
  try {
    obj = JSON.parse(json);
  } catch {
...
function parseTokenDialect(
  ...
  clean = clean
    .replaceAll("<|tool_calls_section_begin|>", "")
    .replaceAll("<|tool_calls_section_end|>", "");

  const callRe =
    /<\|tool_call_begin\|>\s*([\s\S]*?)\s*<\|tool_call_argument_begin\|>\s*/g;
  ...
  while ((m = callRe.exec(clean)) !== null) {
    ...
    const extracted =
      jsonStart === -1 ? null : extractBalancedJson(clean, jsonStart);
    ...
    const endTag = clean.indexOf(
      "<|tool_call_end|>",
      extracted?.end ?? argStart,
    );
    spans.push({
      from: m.index,
      to:
        endTag === -1
          ? (extracted?.end ?? argStart)
          : endTag + "<|tool_call_end|>".length,
    });
  }
  for (const s of [...spans].reverse())
    clean = clean.slice(0, s.from) + clean.slice(s.to);
  return {
    calls: parsedAll && calls.length > 0 ? calls : null,
...
function parseFunctionTagDialect(
  ...
  while ((m = headRe.exec(text)) !== null) {
    ...
    const jsonStart =
      text[afterHead] === "{" || text[afterHead] === "["
        ? afterHead
        : text.indexOf("{", afterHead);
    const extracted =
      jsonStart === -1 ? null : extractBalancedJson(text, jsonStart);
    let ok = false;
    if (
      extracted &&
      isKnownTool(name, toolNames) &&
      extracted.json.startsWith("{")
    ) {
      try {
        JSON.parse(extracted.json);
        ok = true;
    ...
    const closeTag = text.indexOf(
      "</function>",
      extracted?.end ?? m.index + m[0].length,
    );
    spans.push({
      from: m.index,
      to:
 ...
 *                   used by tests only)
 */
export function rescueInlineToolCalls(
  text: string,
  toolNames: Set<string>,
): RescueResult {
