...
export const REGISTRY: FeatureSettingDef[] = [
  ...
  },
  {
    ...
    description: 'Minimum intelligence score required when Smartest routing is active.',
    type: 'number',
    default: 60,
    min: 0,
    max: 100,
  ...
  },
  {
    ...
    description: 'Minimum combined speed/latency required when Fastest routing is active.',
    type: 'number',
    default: 60,
    min: 0,
    max: 100,
  ...
  },
  {
    ...
    description: 'Minimum success rate required when Most Reliable routing is active.',
    type: 'number',
    default: 60,
    min: 0,
    max: 100,
...
export function getAllFeatureSettings(): Array<FeatureSettingDef & { value: boolean | number | string }> {
  return REGISTRY.map((def) => ({ ...def, value: resolveSetting(def) }));
}

/** Validate and write a partial update of settings to the DB. */
export function saveFeatureSettings(updates: Record<string, boolean | number | string>): string[] {
  const errors: string[] = [];
