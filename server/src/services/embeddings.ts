...
export function listEmbeddingModels(): EmbeddingModelRow[] {
  return getDb()
    .prepare("SELECT * FROM embedding_models ORDER BY family, priority")
...
async function callProvider(
  ...
  switch (row.platform) {
    ...
    case "cloudflare": {
      ...
      if (sep === -1)
        throw new EmbeddingsError(
          "cloudflare key is not in account_id:token form",
          500,
        );
    ...
    case "cohere": {
      ...
      return {
        vectors: j.embeddings?.float ?? [],
        inputTokens: j.meta?.billed_units?.input_tokens ?? null,
      };
    }
    default:
      throw new EmbeddingsError(
        `no embeddings adapter for platform '${row.platform}'`,
        500,
      );
  }
}
...
export async function runEmbeddings(
  ...
  if (!family) {
    throw new EmbeddingsError(
      `Unknown embedding model '${model}'. Use 'auto', a family name, or a provider model id.`,
      400,
    );
  }
  ...
  if (chain.length === 0) {
    throw new EmbeddingsError(
      `No providers for embedding family '${family}'.`,
      503,
    );
  }
  ...
  for (const row of chain) {
    ...
    try {
      ...
      if (
        out.vectors.length !== inputs.length ||
        out.vectors.some((v) => !Array.isArray(v) || v.length === 0)
      ) {
        throw new EmbeddingsError(
          "upstream returned malformed embeddings",
          502,
        );
      }
      ...
      const e =
        err instanceof EmbeddingsError
          ? err
          : new EmbeddingsError(String(err?.message ?? err), 502);
      logEmbeddingRequest(
        row,
        "error",
        0,
