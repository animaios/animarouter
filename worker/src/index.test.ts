...
describe("AnimaRouter anonymizing Worker", () => {
  ...
  it("returns health status without auth", async () => {
    const res = await handleRequest(
      new Request("https://relay.example.test/healthz"),
      env,
    );
  ...
  it("rejects non-POST proxy requests", async () => {
    const res = await handleRequest(
      new Request(proxyUrl("https://api.example.test/v1/chat/completions")),
      env,
    );
  ...
  it("rejects invalid auth keys", async () => {
    const res = await handleRequest(
      new Request(
        proxyUrl("https://api.example.test/v1/chat/completions", "wrong"),
        {
          method: "POST",
  ...
  it("rejects malformed encoded upstream paths without calling fetch", async () => {
    ...
    const res = await handleRequest(
      new Request(
        `https://relay.example.test/${env.PROXY_AUTH_KEY}/1/not-base64!`,
        {
          method: "POST",
          headers: { authorization: "Bearer provider-key" },
        },
  ...
  it("supports exact and wildcard host restrictions", () => {
    expect(
      isAllowedHost("api.openai.com", "api.openai.com,*.example.com"),
    ).toBe(true);
    expect(
      isAllowedHost("tenant.example.com", "api.openai.com,*.example.com"),
    ).toBe(true);
    expect(isAllowedHost("example.com", "*.example.com")).toBe(false);
    expect(isAllowedHost("blocked.test", "api.openai.com,*.example.com")).toBe(
      false,
    );
  });
  ...
  it("rejects non-HTTPS upstreams", async () => {
    const res = await handleRequest(
      request("http://api.example.test/v1/chat/completions"),
      env,
    );
    expect(res.status).toBe(400);
  });
  ...
  it("forwards only provider-safe request headers and preserves body", async () => {
    ...
    const res = await handleRequest(
      request("https://api.example.test/v1/chat/completions"),
      env,
    );
