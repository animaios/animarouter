...
This was acceptable when the only consequence of a slow cycle was slightly stale health data. However, the **429 Key Exclusion** spec has changed the stakes:
...
2. **Prewarm startup** runs one initial cycle (`skipGate=true`). A slow sequential prewarm delays the server becoming fully operational.
...
**As an operator**, I want heartbeat cycles to complete in seconds, not minutes, so that evicted keys are restored quickly.
