...
2. **Request-level Rabbit Shake oscillator** (v2 expansion) — for complex reasoning prompts, the router executes a 3-step sequential multi-model pipeline instead of a single model call, injecting divergent reasoning to break logic loops.
...
### 1.3 Key Insight: The Budget Trick (unchanged from v1)
...
1. `oscillator_enabled` feature flag is true
...
### 4.2 Rabbit Shake Oscillator Settings (NEW)
...
| `applyAdvice` | Score boost/penalty caps, cooldown factors, oscillator toggle |
